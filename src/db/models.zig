...
pub fn getAllEducation(db: *sqlite.Db, allocator: std.mem.Allocator) ![]Education {
    ...
    defer stmt.deinit();
    return stmt.all(Education, allocator, .{});
}
...
pub fn getAllExperience(db: *sqlite.Db, allocator: std.mem.Allocator) ![]Experience {
    ...
    defer stmt.deinit();
    return stmt.all(Experience, allocator, .{});
}
...
pub fn getAllProjects(db: *sqlite.Db, allocator: std.mem.Allocator) ![]Project {
    ...
    defer stmt.deinit();
    return stmt.all(Project, allocator, .{});
}
...
pub fn insertSkill(db: *sqlite.Db, s: Skill) !void {
    try db.exec("INSERT INTO skills (category, skills) VALUES (?1, ?2)", .{ s.category, s.skills });
}

pub fn getAllSkills(db: *sqlite.Db, allocator: std.mem.Allocator) ![]Skill {
    var stmt = try db.prepare("SELECT id, category, skills FROM skills");
    defer stmt.deinit();
    return stmt.all(Skill, allocator, .{});
}

pub fn updateSkill(db: *sqlite.Db, s: Skill) !void {
    try db.exec("UPDATE skills SET category=?1, skills=?2 WHERE id=?3", .{ s.category, s.skills, s.id });
}
...
pub fn getAllCertifications(db: *sqlite.Db, allocator: std.mem.Allocator) ![]Certification {
    var stmt = try db.prepare("SELECT id, name, issuer, date, url, description FROM certifications");
    defer stmt.deinit();
    return stmt.all(Certification, allocator, .{});
}

pub fn updateCertification(db: *sqlite.Db, c: Certification) !void {
    try db.exec("UPDATE certifications SET name=?1, issuer=?2, date=?3, url=?4, description=?5 WHERE id=?6", .{ c.name, c.issuer, c.date, c.url, c.description, c.id });
}
